...
for i=1:L1
    for j=1:L2
        ...
        if remainder==1
            solution(i,j)=(i-1)^N_harm*(-1)^((N_harm-3+2*(2-mod(j,2)))/2)*A(i,j+(-1)^(mod(j,2)+1));
        else
            solution(i,j)=(i-1)^N_harm*(-1)^(N_harm/2)*A(i,j);
        end
    end
end
for i=1:L2/2
